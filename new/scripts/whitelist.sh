#!/bin/bash
# whitelist.sh
# Usage: whitelist.sh <IP_or_hostname>
...
IP=$(resolve_ip "$IP")
echo -e "${YELLOW}Processing whitelist for $IP...${NC}"

# t4s allow
if command -v t4s >/dev/null 2>&1; then
    echo -e "${YELLOW}[t4s] Allowing IP...${NC}"
    t4s allow "$IP"
fi

# --- CSF ---
if command -v csf >/dev/null 2>&1; then
    echo -e "${YELLOW}[CSF] Adding to whitelist...${NC}"
...
if command -v imunify360-agent >/dev/null 2>&1; then
    echo -e "${YELLOW}[Imunify360] Adding to whitelist...${NC}"
...
if command -v iptables >/dev/null 2>&1; then
    echo -e "${YELLOW}[iptables] Allowing IP...${NC}"
...
if command -v whmapi1 >/dev/null 2>&1; then
    echo -e "${YELLOW}[cPHulk] Adding to whitelist...${NC}"
    whmapi1 cphulkd_whitelist_add ip="$IP" >/dev/null
    echo -e "${GREEN}[cPHulk] Done.${NC}"
fi

echo -e "${GREEN}✅ IP $IP whitelisted successfully!${NC}"
